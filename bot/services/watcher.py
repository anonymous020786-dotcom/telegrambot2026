...
from .jobs import DuplicateJob, Job, PolicyBlocked, QuotaExceeded
...
async def check_watch(app: Application, row) -> int:
    ...
    for entry in list(reversed(new))[:MAX_NEW_PER_CHECK]:
        ...
        try:
            ...
            queued += 1
        except (PolicyBlocked, DuplicateJob):
            continue
...
async def run_schedule(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await context.bot.send_message(
            row["chat_id"], f"⏰ Scheduled download is already in your queue as job {dup.existing.id}."
        )
